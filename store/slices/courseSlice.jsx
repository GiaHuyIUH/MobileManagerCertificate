...
export const getAllCourses = createAsyncThunk(
  ...
  async (limit = 12, { getState }) => {
    ...
    const response = await axios.get(
      `${process.env.VITE_API_BASE_URL}/course?limit=${limit}&offset=${offset}`
    );
    return response.data;
  }
);
...
export const getAllCourseBundles = createAsyncThunk(
  ...
  async () => {
    const response = await axios.get(`${process.env.VITE_API_BASE_URL}/coursebundles`); // Replace with your actual API endpoint
    return response.data;
  }
);
